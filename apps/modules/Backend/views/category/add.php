	...
	<form name="myform" id="myform"
		...
		<?php endif; ?>
			...
			<tr>
				<th>分类类型：</th>
				<td><select
					<?php echo $parent_cat_info ? 'disabled="disabled"' : ''; ?> id="cat_type" name="cat_type">
     		<?php foreach ($cat_type_list as $type_id => $type_name): ?>
            	<option
							<?php echo ($parent_cat_info && $type_id==$parent_cat_info['cat_type']) ? 'selected="selected"' : ''; ?>
			...
			<tr>
				...
				<td><input type="text" name="out_url" style="width: 250px;"
					id="out_url" class="input-text" /></td>
			</tr>
			<tr>
				<th>是否显示分类：</th>
				<td><input type="radio" name="display" value="1" checked> 是 <input
