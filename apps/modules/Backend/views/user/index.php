...
<div class="subnav">
	<div class="content-menu ib-a blue line-x">
		<a class="add fb"
			href="javascript:postDialog('addUser', '<?php echo YUrl::createBackendUrl('User', 'add'); ?>', '添加用户', 500, 450)"><em>添加用户</em></a>
...
Calendar.setup({
    ...
    <?php foreach ($list as $item): ?>
    	<tr>
                        <td align="center"><img src="<?php echo $item['avatar']; ?>" height="80"></td>
    		  ...
    		  [<a href="###"
							onclick="normalDialog('deleteType', '<?php echo YUrl::createBackendUrl('User', 'unforbid', ['user_id' => $item['user_id']]); ?>', '<?php echo "您确定要解禁[{$item['username']}]用户吗?" ?>')"
							title="解禁">解禁</a>]
    		  <?php else: ?>
    		  [<a href="###"
							onclick="forbid(<?php echo $item['user_id'] ?>, '<?php echo $item['username'] ?>')"
							title="封禁">封禁</a>]
    		  <?php endif; ?>
    		</td>
					</tr>
...
function edit(id, name) {
	...
	postDialog('editUser', page_url, title, 500, 450);
}
