...
class CategoryService extends AbstractService
{
    ...
    const CAT_DOC      = 1;    // 技术资料。
    const CAT_QUESTION = 2;    // 技术题库。
    ...
    public static function getCategoryByCatCode($catCode)
    {
        ...
        if (empty($catInfo)) {
            YCore::exception(STATUS_ERROR, '文章对应的分类不存在');
        }
        return $catInfo;
    }
     ...
     * @return array
     */
    public static function getCategoryList($parentid = 0, $catType = self::CAT_DOC, $isFilter = false)
    {
        ...
        if (empty($categoryList)) {
            ...
            foreach ($categoryList as $key => $menu) {
                $categoryList[$key]['sub'] = self::getCategoryList($menu['cat_id'], $catType, $isFilter);
            }
            return $categoryList;
        }
    }
     ...
     * @return void
     */
    public static function addCategory($adminId, $catType, $catName, $parentid = 0, $isOutUrl = 0, $outUrl = '', $display = 1)
    {
        $CategoryModel = new Category();
        $lv = 1;
        if ($parentid != 0) {
            $parentCatInfo = $CategoryModel->fetchOne([], ['cat_id' => $parentid, 'status' => 1]);
            if (empty($parentCatInfo)) {
                YCore::exception(STATUS_ERROR, '父分类不存在或已经删除');
            }
     ...
     * @return void
     */
    public static function editCategory($adminId, $catId, $catName, $isOutUrl = 0, $outUrl = '', $display = 1)
    {
        $CategoryModel = new Category();
     ...
     * @return void
     */
    public static function deleteCategory($adminId, $catId)
    {
        // [1]
        $CategoryModel = new Category();
        $catInfo       = $CategoryModel->fetchOne([], ['cat_id' => $catId, 'status' => Category::STATUS_NORMAL]);
        if (empty($catInfo)) {
            YCore::exception(STATUS_ERROR, '分类不存在或已经删除');
        }
        // [2] 目前只检查文章与友情链接，后续如果关联了其他功能，这里要做适当调整。
        $NewsModel = new News();
        $newsCount = $NewsModel->count(['cat_code' => $catInfo['cat_code'], 'status' => News::STATUS_NORMAL]);
        if ($newsCount > 0) {
            YCore::exception(STATUS_ERROR, '请先清空该分类下的文章');
        }
        $where = [
            'cat_id' => $catId
        ];
    ...
    {
        $CategoryModel = new Category();
        $data = $CategoryModel->fetchOne([], ['cat_id' => $catId, 'status' => Category::STATUS_NORMAL]);
        if (empty($data)) {
            YCore::exception(STATUS_ERROR, '分类不存在或已经删除');
        }
        return $data;
    ...
    public static function getNewCategoryCode($parentid)
    {
        ...
        if ($parentid == 0) {
            ...
            $data = $CategoryModel->rawQuery($sql, $params)->rawFetchOne();
            if ($data) {
            ...
            $catInfo = $CategoryModel->fetchOne([], ['cat_id' => $parentid, 'status' => 1]);
            if (empty($catInfo)) {
                YCore::exception(STATUS_ERROR, '父分类不存在或已经删除');
            }
    ...
    {
        $cacheKey = 'phpjieshuo_get_all_categorys';
